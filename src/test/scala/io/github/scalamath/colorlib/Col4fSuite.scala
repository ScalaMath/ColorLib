...
class Col4fSuite extends AnyFunSuite {
  ...
  test("Create color from rgba floats and retrieve the values") {
    val color = Col4f(0.372549f, 0.619608f, 0.627451f, 0.8f)
  ...
  test("Create color from rgb floats and retrieve the values") {
    val color = Col4f(0.372549f, 0.619608f, 0.627451f)
    assert(color.r === 0.372549f)
    assert(color.g === 0.619608f)
    assert(color.b === 0.627451f)
    assert(color.a === 1.0f)
  ...
  test("Create color from rgba integers and retrieve the values") {
    val color = Col4f(95, 158, 160, 128)
    assert(color.r8 == 95)
    assert(color.g8 == 158)
    assert(color.b8 == 160)
    assert(color.a8 == 128)
  ...
  test("Create color from HSV and retrieve the values") {
    val color = Col4f.hsv(0.5f, 0.41f, 0.63f)
    assert(color.h === 0.5f)
  ...
  test("Create color from rgba hex and retrieve the value") {
    val color = Col4f.rgba(0x5f9ea0ff)
    assert(color.rgba == 0x5f9ea0ff)
  }

  test("Create color from argb hex and retrieve the value") {
    val color = Col4f.argb(0xff5f9ea0)
    assert(color.argb == 0xff5f9ea0)
  }

  test("Add four floats to a color") {
    val color = Col4f(0.3f, 0.4f, 0.5f, 0.6f)
    val res = Col4f(0.4f, 0.6f, 0.8f, 1.0f)
    assert(color + (0.1f, 0.2f, 0.3f, 0.4f) === res)
  }

  test("Add three floats to a color") {
    val color = Col4f(0.3f, 0.4f, 0.5f, 0.8f)
    val res = Col4f(0.4f, 0.6f, 0.8f, 0.8f)
    assert(color + (0.1f, 0.2f, 0.3f) === res)
  }

  test("Sum of two colors") {
    val c1 = Col4f(0.3f, 0.4f, 0.5f, 0.6f)
  ...
  test("Subtract four floats from a color") {
    val color = Col4f(0.4f, 0.6f, 0.8f, 1.0f)
  ...
  test("Subtract three floats from a color") {
    val color = Col4f(0.4f, 0.6f, 0.8f, 1.0f)
    val res = Col4f(0.3f, 0.4f, 0.5f, 1.0f)
    assert(color - (0.1f, 0.2f, 0.3f) === res)
  }

  test("Subtract two colors") {
    val c1 = Col4f(0.4f, 0.6f, 0.8f, 1.0f)
  ...
  test("Multiply two colors of different types") {
    val c1 = Col4f(2.0f, 3.0f, 1.5f, 1.0f)
    val c2 = Col1i(0.2f, 0.3f, 0.4f, 1.0f)
    val res = Col4f(0.4f, 0.9f, 0.6f, 1.0f)
    assert(c1 * c2 === res)
  }
  ...
  test("Multiply a color with a float") {
    ...
    val res = Col4f(0.3f, 0.6f, 0.9f, 1.5f)
    assert(color * 1.5f === res)
  }

  test("Divide a color by four floats") {
    val color = Col4f(1.0f, 0.8f, 0.6f, 1.0f)
  ...
  test("Divide a color by three floats") {
    val color = Col4f(1.0f, 0.8f, 0.6f, 1.0f)
    val res = Col4f(0.5f, 0.2f, 0.2f, 1.0f)
    assert(color / (2.0f, 4.0f, 3.0f) === res)
  }

  test("Divide two colors") {
    val c1 = Col4f(1.0f, 0.8f, 0.6f, 1.0f)
    val c2 = Col4f(2.0f, 4.0f, 3.0f, 5.0f)
    val res = Col4f(0.5f, 0.2f, 0.2f, 0.2f)
    assert(c1 / c2 === res)
  }
  ...
  test("Divide two colors of different types") {
    val c1 = Col4f(1.0f, 0.8f, 0.6f, 1.0f)
    val c2 = Col1i(0.5f, 0.2f, 0.25f, 1.0f)
  ...
  test("Invert a color") {
    val color = Col4f(0.2f, 0.5f, 0.7f, 1.0f)
    val res = Col4f(0.8f, 0.5f, 0.3f, 1.0f)
  ...
  test("Color equals approx three floats") {
    val color = Col4f(0.9999999f, 0.4999999f, 0.7500001f)
    assert(color ~= (1.0f, 0.5f, 0.75f))
  }

  test("Color equals approx four floats") {
    val color = Col4f(0.9999999f, 0.4999999f, 0.7500001f, 0.9999999f)
    assert(color ~= (1.0f, 0.5f, 0.75f, 1.0f))
  }

  test("Color equals approx another color") {
    val c1 = Col4f(0.9999999f, 0.4999999f, 0.7500001f, 0.9999999f)
    val c2 = Col4f(1.0f, 0.5f, 0.75f, 1.0f)
    assert(c1 ~= c2)
  }
